...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        ...
        =======================================
        return Pandas DataFrame if atleast  a single column is avaiable after missing column drop else None
        """
        try:
            
            logging.info(f"droping_missing_values_columns in the dataframe df.")
            threshold = self.data_validation_config.missing_threshold
            null_report = df.isna().sum()/df.shape[0]
            # selecting columns names which contains null values certain percentage
            logging.info(f"selecting columns names which contains null above to {threshold}")
            drop_column_names = null_report[null_report>threshold].index

            self.Validation_error[report_key_name]= list(drop_column_names)
            logging.info(f"droped columns are: {list(drop_column_names)}")
            df.drop(list(drop_column_names),axis=1,inplace=True)

            #return None no columns left
            if len(df.columns)==0:
                logging.info(f"returning None if no columns left.")
                return None
            return df           
        except Exception as e:
            raise SensorException(e,sys)
    ...
    def is_required_colums_exists(Self,base_df:pd.DataFrame,current_df:pd.DataFrame,report_key_name:str)->bool:
        try:
            logging.info(f"is_required_columns_exists or not.")
            ...
            logging.info(f"missing_columns are:{missing_columns}")
        ...
        try:
            ...
            for base_column in base_columns:
                ...
                if same_distribution.pvalue>0.05:
                    #Same distribution
                    #We ate acception null hypothesis
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info(f"replacing na with np.nan and droping missing values columns")
            ...
            if train_df_columns_status:
                ...
                self.data_drift(base_df=base_df, current_df=train_df,report_key_name="data__within_train_dataset")
            if test_df_columns_status:
                logging.info(f"As all columns available in test df hence detecting data drift")
                self.data_drift(base_df=base_df, current_df=test_df,report_key_name="data__within_test_dataset")
